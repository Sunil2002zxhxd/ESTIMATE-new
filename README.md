...
#savedTable input.status,#savedTable input.particular{
  border:none;
  background:#f8f8f8;
  width:100%;
}
...
<table id="savedTable">
<thead><tr><th>#</th><th>Customer</th><th>Particular</th><th>Total</th><th>Status</th><th>Delete</th></tr></thead>
<tbody></tbody>
</table>
</div>

<script>
const STORAGE_KEY='mohammadi_estimates_final';
...
function refreshTable(){
  ...
  getStored().forEach((e,i)=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${e.estNo}</td>
    <td>${e.customer}</td>
    <td><input class="particular" value="${e.items[0]?.part||''}" oninput="updateParticular(${i},this.value)"></td>
    <td>₹${e.total}</td>
...
function updateStatus(i,v){const arr=getStored();if(arr[i])arr[i].status=v;setStored(arr);}
function updateParticular(i,v){const arr=getStored();if(arr[i])arr[i].items[0].part=v;setStored(arr);}
function delEstimate(i){const arr=getStored();arr.splice(i,1);setStored(arr);refreshTable();}
